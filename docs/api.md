...
REST API that classifies natural language input into GTD buckets and files the results as Markdown notes in an Obsidian vault. Built with Spring Boot 3 + Spring AI + Groq (Llama 3.3-70b).
...
One message can contain multiple tasks — create, done, update, move, edit, and dismiss operations are supported in a single call.
...
Items classified as `discard` are not filed but logged to `.vault-meta/discard-log.jsonl`. A two-level prompt strategy handles ambiguous input: a lightweight prompt runs first, and falls back to a more detailed one if the response fails to parse or everything comes back as `discard`/`now`. When the fallback triggers, `fallback: true` is returned:
...
| `POST` | `/api/items/{filename}/move` | Reclassify an item to another bucket — `{ "bucket": "...", "due": "YYYY-MM-DD" }` |
...
| `PUT` | `/api/items/{filename}/meta` | Update metadata fields — `{ "title", "tags", "due", "today_since" }`. Returns `{ updated: true, file }` |
| `POST` | `/api/items/{filename}/markdownify` | AI-enrich an item: rewrites body and infers tags via LLM. Returns `{ file, body, tags }` |
| `GET` | `/api/review` | Weekly review stats. Params: `staleDays` (default 3), `dueDays` (default 7), `completedDays` (default 7). Returns `{ stale_today, due_this_week, completed_this_week, week_stats }` |
| `POST` | `/api/transcribe` | Transcribe audio to text via Groq Whisper — multipart/form-data with `audio` field, returns `{ text }` |
