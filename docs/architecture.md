...
- **Bounded open-tasks context** — before serializing the open tasks into the prompt, a keyword pre-filter (`filterRelevantTasks`) keeps only the ~15 tasks whose titles overlap the message, so a large vault doesn't blow the token budget or bury the task a follow-up op refers to. Tasks that share no word with the message (typical of a plain `create`) fall back to original list order rather than being dropped, so a `done`/`edit`/`move`/`dismiss` target is never filtered out. A coarser 6000-char/80-item truncation remains as a final safety net.
